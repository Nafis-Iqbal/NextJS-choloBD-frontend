import React, { useMemo, useState } from "react";
import { BookingStatus, HotelRoomCategory } from "@/types/enums";
import {
  CustomDatePicker,
  CustomTextInput,
} from "@/components/custom-elements/CustomInputElements";

const toLocalDateKey = (date: Date): string => {
  const yyyy = date.getFullYear();
  const mm = String(date.getMonth() + 1).padStart(2, "0");
  const dd = String(date.getDate()).padStart(2, "0");
  return `${yyyy}-${mm}-${dd}`;
};

const getTodayKey = (): string => toLocalDateKey(new Date());

const getTomorrowKey = (): string => {
  const tomorrow = new Date();
  tomorrow.setDate(tomorrow.getDate() + 1);
  return toLocalDateKey(tomorrow);
};

const toDateKey = (date: Date | string | undefined): string => {
  if (!date) return "";
  if (typeof date === "string") {
    const match = date.match(/^(\d{4}-\d{2}-\d{2})/);
    if (match) return match[1];
  }
  const d = new Date(date);
  if (Number.isNaN(d.getTime())) return "";
  return toLocalDateKey(d);
};

// Room Bookings Component
export const HotelRoomBookingsManagement: React.FC<{
  bookings: HotelRoomBooking[];
  className?: string;
}> = ({ bookings, className }) => {
  const [selectedBooking, setSelectedBooking] = useState<string | null>(null);
  const [checkInFilter, setCheckInFilter] = useState(getTodayKey);
  const [checkOutFilter, setCheckOutFilter] = useState(getTomorrowKey);
  const [customerNameFilter, setCustomerNameFilter] = useState("");

  const formatDate = (date: Date | string | undefined) => {
    if (!date) return "N/A";
    const d = new Date(date);
    return d.toLocaleDateString("en-US", {
      year: "numeric",
      month: "short",
      day: "numeric",
    });
  };

  const formatRoomTypeName = (type: string) => {
    return type
      .split("_")
      .map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
      .join(" ");
  };

  const getRoomTypeColor = (type: string) => {
    const typeMap: Record<string, string> = {
      [HotelRoomCategory.SINGLE]:
        "bg-blue-600/30 text-blue-800 border border-blue-500",
      [HotelRoomCategory.DOUBLE]:
        "bg-purple-600/30 text-purple-800 border border-purple-500",
      [HotelRoomCategory.SUITE]:
        "bg-pink-600/30 text-pink-800 border border-pink-500",
      [HotelRoomCategory.DELUXE]:
        "bg-green-600/30 text-green-800 border border-green-500",
    };

    return (
      typeMap[type] || "bg-gray-600/30 text-gray-300 border border-gray-500"
    );
  };

  const getRoomTypesSummary = (
    roomDetails: HotelRoomBookingDetail[] | undefined
  ) => {
    if (!roomDetails || roomDetails.length === 0) {
      return (
        <span className="text-xs" style={{ color: "var(--theme-text-subtle)" }}>
          No rooms booked
        </span>
      );
    }

    const roomTypeCounts = roomDetails.reduce(
      (acc, detail) => {
        const roomType = detail.hotelRoom?.roomType || "Unknown";
        acc[roomType] = (acc[roomType] || 0) + 1;
        return acc;
      },
      {} as Record<string, number>
    );

    return (
      <div className="flex flex-wrap gap-2 mt-1">
        {Object.entries(roomTypeCounts).map(([type, count]) => (
          <span
            key={type}
            className={`px-2 py-1 rounded-md text-xs font-medium ${getRoomTypeColor(type)}`}
          >
            {formatRoomTypeName(type)}{" "}
            <span className="font-bold">×{count}</span>
          </span>
        ))}
      </div>
    );
  };

  const getStatusColor = (status: BookingStatus) => {
    switch (status) {
      case BookingStatus.PENDING:
        return "bg-yellow-600/30 text-yellow-800 border border-yellow-600/50";
      case BookingStatus.CONFIRMED:
        return "bg-blue-600/30 text-blue-800 border border-blue-600/50";
      case BookingStatus.COMPLETED:
        return "bg-green-600/30 text-green-800 border border-green-600/50";
      case BookingStatus.CANCELLED:
        return "bg-red-600/30 text-red-800 border border-red-600/50";
      case BookingStatus.REFUNDED:
        return "bg-purple-600/30 text-purple-800 border border-purple-600/50";
      case BookingStatus.NO_SHOW:
        return "bg-orange-600/30 text-orange-800 border border-orange-600/50";
      default:
        return "bg-gray-600/30 text-gray-800 border border-gray-600/50";
    }
  };

  const getStatusColorVar = (status: BookingStatus): string => {
    switch (status) {
      case BookingStatus.PENDING:
        return "var(--theme-yellow, rgb(234, 179, 8))";
      case BookingStatus.CONFIRMED:
        return "var(--theme-blue, rgb(59, 130, 246))";
      case BookingStatus.COMPLETED:
        return "var(--theme-green, rgb(34, 197, 94))";
      case BookingStatus.CANCELLED:
        return "var(--theme-red, rgb(220, 53, 69))";
      case BookingStatus.REFUNDED:
        return "var(--theme-purple, rgb(147, 51, 234))";
      case BookingStatus.NO_SHOW:
        return "var(--theme-orange, rgb(239, 68, 68))";
      default:
        return "var(--theme-text-muted)";
    }
  };

  const filteredBookings = useMemo(() => {
    const nameQuery = customerNameFilter.trim().toLowerCase();

    return bookings.filter((booking) => {
      if (checkInFilter && toDateKey(booking.checkInDate) !== checkInFilter) {
        return false;
      }
      if (checkOutFilter && toDateKey(booking.checkOutDate) !== checkOutFilter) {
        return false;
      }
      if (nameQuery) {
        const guestName = (booking.guestName || "").toLowerCase();
        if (!guestName.includes(nameQuery)) {
          return false;
        }
      }
      return true;
    });
  }, [bookings, checkInFilter, checkOutFilter, customerNameFilter]);

  const statusCounts = useMemo(() => {
    const counts: Record<"total" | BookingStatus, number> = {
      total: filteredBookings.length,
      [BookingStatus.PENDING]: 0,
      [BookingStatus.ACCEPTED]: 0,
      [BookingStatus.DECLINED]: 0,
      [BookingStatus.CONFIRMED]: 0,
      [BookingStatus.COMPLETED]: 0,
      [BookingStatus.CANCELLED]: 0,
      [BookingStatus.REFUNDED]: 0,
      [BookingStatus.NO_SHOW]: 0,
    };

    filteredBookings.forEach((booking) => {
      counts[booking.status]++;
    });

    return counts;
  }, [filteredBookings]);

  const roomTypeCounts = useMemo(() => {
    const counts: Record<string, number> = {
      [HotelRoomCategory.SINGLE]: 0,
      [HotelRoomCategory.DOUBLE]: 0,
      [HotelRoomCategory.SUITE]: 0,
      [HotelRoomCategory.DELUXE]: 0,
    };

    filteredBookings.forEach((booking) => {
      booking.roomDetails?.forEach((detail) => {
        const roomType = detail.hotelRoom?.roomType;
        if (roomType) {
          counts[roomType] = (counts[roomType] || 0) + 1;
        }
      });
    });

    return counts;
  }, [filteredBookings]);

  const defaultCheckIn = getTodayKey();
  const defaultCheckOut = getTomorrowKey();
  const hasActiveFilters =
    checkInFilter !== defaultCheckIn ||
    checkOutFilter !== defaultCheckOut ||
    !!customerNameFilter.trim();

  const filterLabelStyle = "text-xs font-medium theme-text-teal";
  const filterControlClassName =
    "min-w-[120px] !h-[34px] text-xs [&_span]:!text-xs [&_span.font-semibold]:!text-sm";

  return (
    <section
      className={`mb-8 ${className || ""}`}
      id="hotel_room_bookings_management"
    >
      <div className="mb-4 space-y-3">
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3">
          <div className="min-w-0 flex-1">
            <h2
              className="text-xl sm:text-2xl font-bold"
              style={{ color: "var(--theme-text)" }}
            >
              Room Bookings
            </h2>
            <div
              className="mt-2 flex flex-wrap items-baseline gap-x-1 gap-y-1 text-sm sm:text-base"
              style={{ color: "var(--theme-text-muted)" }}
            >
              <span>
                Total:{" "}
                <span
                  className="font-semibold"
                  style={{ color: "var(--theme-teal)" }}
                >
                  {statusCounts.total}
                </span>
              </span>
              {statusCounts[BookingStatus.PENDING] > 0 && (
                <span>
                  {" · "}Pending:{" "}
                  <span
                    className="font-semibold"
                    style={{ color: getStatusColorVar(BookingStatus.PENDING) }}
                  >
                    {statusCounts[BookingStatus.PENDING]}
                  </span>
                </span>
              )}
              {statusCounts[BookingStatus.CONFIRMED] > 0 && (
                <span>
                  {" · "}Confirmed:{" "}
                  <span
                    className="font-semibold"
                    style={{
                      color: getStatusColorVar(BookingStatus.CONFIRMED),
                    }}
                  >
                    {statusCounts[BookingStatus.CONFIRMED]}
                  </span>
                </span>
              )}
              {statusCounts[BookingStatus.COMPLETED] > 0 && (
                <span>
                  {" · "}Completed:{" "}
                  <span
                    className="font-semibold"
                    style={{
                      color: getStatusColorVar(BookingStatus.COMPLETED),
                    }}
                  >
                    {statusCounts[BookingStatus.COMPLETED]}
                  </span>
                </span>
              )}
              {statusCounts[BookingStatus.CANCELLED] > 0 && (
                <span>
                  {" · "}Cancelled:{" "}
                  <span
                    className="font-semibold"
                    style={{
                      color: getStatusColorVar(BookingStatus.CANCELLED),
                    }}
                  >
                    {statusCounts[BookingStatus.CANCELLED]}
                  </span>
                </span>
              )}
              {statusCounts[BookingStatus.REFUNDED] > 0 && (
                <span>
                  {" · "}Refunded:{" "}
                  <span
                    className="font-semibold"
                    style={{
                      color: getStatusColorVar(BookingStatus.REFUNDED),
                    }}
                  >
                    {statusCounts[BookingStatus.REFUNDED]}
                  </span>
                </span>
              )}
              {statusCounts[BookingStatus.NO_SHOW] > 0 && (
                <span>
                  {" · "}No Show:{" "}
                  <span
                    className="font-semibold"
                    style={{ color: getStatusColorVar(BookingStatus.NO_SHOW) }}
                  >
                    {statusCounts[BookingStatus.NO_SHOW]}
                  </span>
                </span>
              )}
              {Object.entries(roomTypeCounts)
                .filter(([, count]) => count > 0)
                .map(([type, count]) => (
                  <span key={type}>
                    {" · "}
                    {formatRoomTypeName(type)}:{" "}
                    <span
                      className="font-semibold"
                      style={{ color: "var(--theme-teal)" }}
                    >
                      {count}
                    </span>
                  </span>
                ))}
            </div>
          </div>

          <div className="flex flex-row md:flex-row flex-wrap gap-2 lg:justify-end shrink-0 items-end text-xs">
            <CustomDatePicker
              label="Check-in Date"
              labelStyle={filterLabelStyle}
              value={checkInFilter}
              onChange={setCheckInFilter}
              className={filterControlClassName}
            />
            <CustomDatePicker
              label="Check-out Date"
              labelStyle={filterLabelStyle}
              value={checkOutFilter}
              onChange={setCheckOutFilter}
              className={filterControlClassName}
            />
            <CustomTextInput
              label="Customer Name"
              labelStyle={filterLabelStyle}
              value={customerNameFilter}
              onChange={(e) => setCustomerNameFilter(e.target.value)}
              placeholderText="Search guest name"
              className="min-w-[150px] sm:min-w-[170px] !h-[34px] text-xs"
            />
            {hasActiveFilters && (
              <button
                type="button"
                onClick={() => {
                  setCheckInFilter(getTodayKey());
                  setCheckOutFilter(getTomorrowKey());
                  setCustomerNameFilter("");
                }}
                className="px-3 py-1.5 rounded-lg text-xs font-medium transition-colors h-[34px]"
                style={{
                  backgroundColor: "var(--theme-section-bg)",
                  color: "var(--theme-text)",
                }}
              >
                Clear
              </button>
            )}
          </div>
        </div>
      </div>

      {bookings.length === 0 ? (
        <div
          className="border rounded-xl p-6 text-center"
          style={{
            backgroundColor: "var(--theme-card-bg)",
            borderColor: "var(--theme-deep-green)",
            color: "var(--theme-text-subtle)",
          }}
        >
          No bookings found
        </div>
      ) : filteredBookings.length === 0 ? (
        <div
          className="border rounded-xl p-6 text-center"
          style={{
            backgroundColor: "var(--theme-card-bg)",
            borderColor: "var(--theme-deep-green)",
            color: "var(--theme-text-subtle)",
          }}
        >
          No bookings match the current filters
        </div>
      ) : (
        <div
          className="rounded-xl overflow-y-auto max-h-[80vh] min-h-[40vh] p-3 sm:p-4"
          style={{
            backgroundColor: "var(--theme-card-bg)",
            border: "1px solid var(--theme-deep-green)",
          }}
        >
          <div className="space-y-2">
            {filteredBookings.map((booking) => (
              <div
                key={booking.id}
                className="rounded-lg p-3 sm:p-4 overflow-hidden transition-colors"
                style={{
                  backgroundColor: "var(--theme-bg)",
                  border: "1px solid var(--theme-deep-green)",
                }}
              >
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between min-w-0">
                  <div className="flex-1 min-w-0">
                    <p
                      className="font-semibold break-words"
                      style={{ color: "var(--theme-text)" }}
                    >
                      {booking.guestName || "Guest"}
                    </p>
                    <p
                      className="text-sm mt-1"
                      style={{ color: "var(--theme-text-muted)" }}
                    >
                      Check-in: {formatDate(booking.checkInDate)} | Check-out:{" "}
                      {formatDate(booking.checkOutDate)}
                    </p>
                    <div className="mt-2">
                      {getRoomTypesSummary(booking.roomDetails)}
                    </div>
                  </div>
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    <div className="text-left sm:text-right">
                      <p
                        className="font-semibold"
                        style={{ color: "var(--theme-text)" }}
                      >
                        ৳ {booking.totalPrice}
                      </p>
                      <span
                        className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(
                          booking.status
                        )}`}
                      >
                        {booking.status === BookingStatus.PENDING
                          ? "⏳ Pending"
                          : booking.status === BookingStatus.CONFIRMED
                            ? "📅 Confirmed"
                            : booking.status === BookingStatus.COMPLETED
                              ? "✓ Completed"
                              : booking.status === BookingStatus.CANCELLED
                                ? "❌ Cancelled"
                                : booking.status === BookingStatus.REFUNDED
                                  ? "💰 Refunded"
                                  : "⚠️ No Show"}
                      </span>
                    </div>
                    <button
                      onClick={() =>
                        setSelectedBooking(
                          selectedBooking === booking.id ? null : booking.id
                        )
                      }
                      className="px-3 py-2 rounded-lg text-sm font-medium transition-colors"
                      style={{
                        backgroundColor: "var(--theme-section-bg)",
                        color: "var(--theme-text)",
                      }}
                    >
                      {selectedBooking === booking.id ? "Hide" : "Actions"}
                    </button>
                  </div>
                </div>

                {selectedBooking === booking.id && (
                  <div
                    className="mt-4 pt-4"
                    style={{
                      borderTopColor: "var(--theme-deep-green)",
                      borderTopWidth: "1px",
                    }}
                  >
                    <div className="mb-4">
                      <h4
                        className="font-semibold mb-3"
                        style={{ color: "var(--theme-text)" }}
                      >
                        Booked Rooms:
                      </h4>
                      <div
                        className="rounded-lg p-3 space-y-2"
                        style={{
                          backgroundColor: "var(--theme-section-bg)",
                        }}
                      >
                        {booking.roomDetails &&
                        booking.roomDetails.length > 0 ? (
                          booking.roomDetails.map((detail) => (
                            <div
                              key={detail.id}
                              className="flex justify-between items-start text-sm gap-2"
                            >
                              <div className="min-w-0">
                                <p style={{ color: "var(--theme-text)" }}>
                                  <span className="font-medium">
                                    Room {detail.hotelRoom?.roomNumber || "N/A"}
                                  </span>{" "}
                                  <span
                                    style={{ color: "var(--theme-text-muted)" }}
                                  >
                                    (
                                    {detail.hotelRoom?.roomType
                                      ? formatRoomTypeName(
                                          detail.hotelRoom.roomType
                                        )
                                      : "Unknown"}
                                    )
                                  </span>
                                </p>
                                <p
                                  className="text-xs mt-1"
                                  style={{ color: "var(--theme-text-subtle)" }}
                                >
                                  ৳ {detail.pricePerNight.toLocaleString()} per
                                  night
                                </p>
                              </div>
                              <p
                                className="font-medium shrink-0"
                                style={{ color: "var(--theme-teal)" }}
                              >
                                ৳ {detail.subtotal.toLocaleString()}
                              </p>
                            </div>
                          ))
                        ) : (
                          <p
                            className="text-sm"
                            style={{ color: "var(--theme-text-muted)" }}
                          >
                            No room details available
                          </p>
                        )}
                      </div>
                    </div>

                    {(booking.guestEmail || booking.guestPhoneNumber) && (
                      <div className="mb-4 text-sm">
                        <h4
                          className="font-semibold mb-2"
                          style={{ color: "var(--theme-text)" }}
                        >
                          Guest Contact:
                        </h4>
                        {booking.guestEmail && (
                          <p
                            className="break-words"
                            style={{ color: "var(--theme-text)" }}
                          >
                            Email: {booking.guestEmail}
                          </p>
                        )}
                        {booking.guestPhoneNumber && (
                          <p style={{ color: "var(--theme-text)" }}>
                            Phone: {booking.guestPhoneNumber}
                          </p>
                        )}
                      </div>
                    )}

                    {booking.specialRequests && (
                      <div className="mb-4 text-sm">
                        <h4
                          className="font-semibold mb-2"
                          style={{ color: "var(--theme-text)" }}
                        >
                          Special Requests:
                        </h4>
                        <p
                          className="rounded p-2 break-words"
                          style={{
                            color: "var(--theme-text)",
                            backgroundColor: "var(--theme-section-bg)",
                          }}
                        >
                          {booking.specialRequests}
                        </p>
                      </div>
                    )}

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                      {booking.status === BookingStatus.CONFIRMED && (
                        <button
                          className="px-4 py-2 rounded-lg font-medium"
                          style={{
                            backgroundColor: "var(--theme-teal)",
                            color: "white",
                          }}
                        >
                          ✓ Check In
                        </button>
                      )}

                      {booking.status === BookingStatus.COMPLETED && (
                        <button
                          className="px-4 py-2 rounded-lg font-medium"
                          style={{
                            backgroundColor: "var(--theme-section-bg)",
                            color: "var(--theme-text)",
                          }}
                        >
                          📋 View Details
                        </button>
                      )}

                      {booking.status !== BookingStatus.COMPLETED &&
                        booking.status !== BookingStatus.CANCELLED && (
                          <button
                            className="px-4 py-2 rounded-lg font-medium"
                            style={{
                              backgroundColor: "rgba(220, 53, 69, 0.3)",
                              color: "rgb(220, 53, 69)",
                            }}
                          >
                            ❌ Cancel Booking
                          </button>
                        )}
                    </div>
                  </div>
                )}
              </div>
            ))}
          </div>
        </div>
      )}
    </section>
  );
};
